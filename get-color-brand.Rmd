...
<br>
<center>
<a href="https://datawizart.com/"><img src="../../kpop_winner/dw_logo-05.png" width=5%></a>
</center>
<style type="text/css">
h1.title {
  display:none;
...
<br>
<center><p>By Dio Ariadi | [datawizart.com](https://datawizart.com/), January 2022</p></center>
<br>
...
```{r call library and data, message=FALSE, warning=FALSE, include=FALSE}
library(tidyverse)
library(grDevices)
library(plotwidgets)
# library(crosstalk)
...
Each dot represents the top 10 colors at a certain top. From here what is the most visible is how Chanel has a smaller area compared to others such as Gucci where this brand consistently has bold colors in its fashion. Second, we can see how the Louis Vuitton color scheme has changed over the years. After 2018 the colors will be brighter and bolder. Lastly Prada, where in 2021 has more subtle colors.

```{r brand and year,fig.align='center',fig.height=10,fig.width=10}
df_analysis %>% 
  # filter() %>%
  ggplot(aes(x=a,y=b))+
...
```{r brand and year split season,fig.align='center',fig.height=8,fig.width=10}
df_analysis %>% 
...
```{r lightness between spring and fall}
df_analysis %>% 
  # filter(type_of_season=="SPRING") %>%
  ggplot(aes(x=type_of_season,y=lightness))+
  # geom_violin()+
  geom_boxplot()+
  # geom_point(aes(color=col_hex),shape=5,size=1)+
  facet_grid(~brand)+
...
df_analysis %>% 
  # filter(type_of_season=="SPRING") %>%
  ggplot(aes(x=type_of_season,y=saturation))+
  # geom_violin()+
  geom_boxplot()+
  # geom_point(aes(color=col_hex),shape=5,size=1)+
  facet_grid(~brand)+
  scale_y_continuous(labels = scales::percent)+
  labs(x="Type of Season",y="Saturation")
  # scale_color_identity()
```


```{r}
df_analysis %>% group_by(brand,year_fix) %>% 
  summarise(median=quantile(hue,c(0.5))) #%>% 
  pivot_wider(names_from = year_fix,values_from = median)
```

```{r saturation distribution}

df_analysis %>% 
  ggplot(aes(x=year_fix,y=saturation,fill=brand,color=brand))+geom_boxplot(position=position_dodge2(padding=0.2),alpha=0.6
...
Color can be extracted and we can see the difference between brands and years. In this example we can see the most visible is how Chanel has a smaller range of color compared to others such as Gucci where this brand consistently has bold colors in its fashion. Also, we can see how the Louis Vuitton color scheme has changed over the years, where after 2018 the colors will be brighter and bolder. Lastly Prada, where in 2021 has more subtle colors. The methodlogy itself can be improved if we can differentiate skin or backgrounds color more precisely.
